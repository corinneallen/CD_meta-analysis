...
studies <- read_xlsx("Table_S2-1.xlsx")%>%
...
term_library <- read_csv("Table_S2-2.csv")
...
db_pre <- read_xlsx("all_db.c_pre_v2.xlsx")%>%
  filter(symbionts!="O") #removing samples that are not CD.

#db <-read_xlsx("Table_S2-3_database.xlsx")
```
...
mm_removals <- mm_diff%>%
  ...
  mutate(selected_method=case_when((index=="523" & method=="DGGE") ~ FALSE,
...
#Cleaned database
```{r}
#Removing duplicate records which would be the same colonies but different methods or markers
db <- db_pre%>%
  left_join(mm_diff_choice)%>%
...
summary_counts <- db %>% 
  summarise(n_index = n_distinct(index),
            ...
            n_country = n_distinct(country_cleaned),
...
method_marker_fig <- marker_supp / method_supp
ggsave("method_marker_fig.svg", method_marker_fig, height=8, width=12)
...
# Checking to see if there is a relationship between year and number of articles included. Exponential model had lower AIC (16) than liner model (123).
...
figure_2b <- ggplot()+
  geom_bar(data=spatial_articles, aes(x=reorder(country_cleaned, n_articles),y=n_articles), fill=   "#d1ce9d" , stat="identity")+
  geom_point(data=spatial_colonies, aes(x = reorder(country_cleaned, sum), y=sum/100), color=  "#a39193")+
  geom_line(data=spatial_colonies, aes(x = reorder(country_cleaned, sum), y=sum/100, group=1), color=  "#a39193")+
  ylim(0,80)+
...
#Studies with high sensitivity techniques and multiple samples taken
...
breaks = c("Cladocopium", "Cladocopium  + Other", "Durusdinium", "Durusdinium + Other", "Cladocopium  + Durusdinium", "Cladocopium  + Durusdinium + Other")

x <- ggplot(gold_standard_symbionts, aes(x=reorder(symbionts, -n), y=n, fill=symbionts_name))+
  geom_bar(stat="identity")+
  xlab("Symbiont presence")+ylab("Number of colonies")+
  scale_fill_manual(values = c("#f6e0b5", "#66545e", "#ce796b"), name="Symbiont presence")+
  theme_classic()+
...
v <- orchaRd::orchard_plot(HetModel,xlab = "Prevalence", mod="method", group="index",angle = 45,g = FALSE, 
...
# Remove sequences below relative abundance threshold. See below section on CD:D ratio by filtering threshold
filt_thresh <- 0.0103
